...
class HeuristicPlayer(BasePokerPlayer):  # Do not forget to make parent class as "BasePokerPlayer"
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if (self.__community_card != []):
            comCards = self.__community_card
            board = []
            '''
            evaluator = de.Evaluator()
            de.Card.print_pretty_cards(self.__community_card)
            '''
            for i in range (len(comCards)):
                hand = comCards[i][::-1]
                hand = hand.replace(hand[1], hand[1].lower())
                board *= de.Card.new(hand)
        action, amount = getPreFlopAction(valid_actions)  # action returned here is sent to the poker engine
...
def getPreFlopAction(valid_actions):
